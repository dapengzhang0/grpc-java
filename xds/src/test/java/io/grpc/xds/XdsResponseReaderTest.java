...
import static io.grpc.xds.XdsClient.EDS_TYPE_URL;
...
import io.grpc.xds.XdsClient.EndpointWatchers;
...
public class XdsResponseReaderTest {
  ...
  public void onResponse() {
    ...
    verify(endpointWatcher1, never()).onEndpointUpdate(any(ClusterLoadAssignment.class));
    verify(endpointWatcher2, never()).onEndpointUpdate(any(ClusterLoadAssignment.class));
  }
  ...
  public void onEdsResponse() {
    ...
    verify(streamActivityWatcher).responseReceived();
    verify(endpointWatcher2).onEndpointUpdate(clusterLoadAssignment);
    verify(endpointWatcher1, never()).onEndpointUpdate(any(ClusterLoadAssignment.class));
  }
