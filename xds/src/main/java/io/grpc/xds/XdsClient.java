...
final class XdsClient {
  static Logger logger = Logger.getLogger(XdsClient.class.getName());
  ...
  XdsClient(
    ...
    syncCtx.executeLater(
        new Runnable() {
          @Override
          public void run() {
            logger.log(Level.INFO, "Starting XdsClient {0}", XdsClient.this);
    ...
    void onEndpointUpdate(ClusterLoadAssignment clusterLoadAssignment) {
      ...
      if (endpointWatchers.containsKey(cluster)) {
        for (EndpointWatcher endpointWatcher : endpointWatchers.get(cluster)) {
          endpointWatcher.onEndpointUpdate(clusterLoadAssignment);
        }
      }
    }
